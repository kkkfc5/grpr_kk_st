...
void update(linkedClient& clientList, linkedCommission& commissionList) {
    ...
    updatePriority(commissionList);
    updateProgress(clientList, commissionList);
}
...
void updatePriority(linkedCommission& commissionList) {
    char choiceYN;
    int comisID, priority;
    bool good = false;

    cout << "\n\nDo you want to update the priority of a commission? (Y/N)\t\t[Q to Quit]\n";
    cin >> choiceYN;
    if (toupper(choiceYN) == 'Y' || toupper(choiceYN) == 'N') { good = true; }
    else if (toupper(choiceYN) == 'Q') { cout << "\n\nQuiting . . . \n\n\n"; return; } // if user wants to quit, end the function.
    ...
    if (toupper(choiceYN) == 'Y') {
        ...
        while (!good) {
            ...
            try {
                cout << "\n\nWhat is the commission ID that you want to update?\n";
                cin >> input;
                comisID = stoi(input);
                if (commissionList.hasCommissionID(comisID)) { good = true; }
                else { cout << "Invalid commission ID\n\n"; }
        ...
        try {
            while (!good) {
                cout << "\n\nWhat do you want the priority to be?\n";
                cin >> input;
                priority = stoi(input);

                if (priority < 0 || priority > 10) { cout << "\n\nInvalid Priority. Try again.\n"; }
                else { good = true; }
    ...
    if (good) {
        commissionList.setPriority(comisID, priority);
    }

}
void updateProgress(linkedClient& clientList, linkedCommission& commissionList) {
    char choiceYN;
    int comisID, progress;
    bool good = false;

    cout << "\n\nDo you want to update the progress of a commission? (Y/N)\t\t[Q to Quit]\n";
    cin >> choiceYN;
    if (toupper(choiceYN) == 'Y' || toupper(choiceYN) == 'N') { good = true; }
    else if (toupper(choiceYN) == 'Q') { cout << "\n\nQuiting . . . \n\n\n"; return; } // if user wants to quit, end the function.
    ...
    if (toupper(choiceYN) == 'Y') {
        ...
        while (!good) {
            ...
            try {
                cout << "\n\nWhat is the commission ID that you want to updat?\n";
                cin >> input;
                comisID = stoi(input);
                if (commissionList.hasCommissionID(comisID)) { good = true; }
                else { cout << "Invalid commission ID\n\n"; }
        ...
        try {
            while (!good) {
                cout << "\n\nWhat do you want the progress to be?\n";
                cin >> input;
                progress = stoi(input);

                if (progress < 0 || progress > 100) { cout << "\n\nInvalid progress. Try again.\n"; }
                else { good = true; }
    ...
    if (good) {
        commissionList.setPriority(comisID, progress);
        if (progress == 100) {
            commissionList.setFinished(comisID);
            clientList.delCommission(comisID);
        }
    }
}